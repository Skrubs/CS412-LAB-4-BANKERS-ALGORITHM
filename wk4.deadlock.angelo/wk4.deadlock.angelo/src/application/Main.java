...
public class Main {
	...
	private HashMap<String, List<Integer>> needHash = new HashMap<>();
	...
	public void setTotal(List<String> file) {
		int count = 0;
		for (String s : file) {
		...
		for (int i = count; i < count + numberOfResources; i++) {

			temp = file.get(i).split(",");
	...
	/**
	 * READS IN THE ALLOCATION MATRIX FROM THE INPUT FILE AND CONVERTS IT TO A 2D
	 * MATRIX
	 * 
	 * @param numberOfProcess
	 * @param numberOfResources
	...
	/**
	 * CREATES A HASMAP FOR PROCESSOR NEEDS
	 * 
	 * @param max
	 * @param allocation
	 */
	public void createNeedHash(int[][] max, int[][] allocation) {
		...
		for (int i = 0; i < max.length; i++) {
			...
			String keyName = "p" + i;
			needHash.put(keyName, new ArrayList<Integer>());

			for (int j = 0; j < max[i].length; j++) {

				num = max[i][j] - allocation[i][j];
				needHash.get(keyName).add(num);
	...
	public static void main(String[] args) {
		...
		m.createNeedHash(m.max, m.allocation);
		...
		// PROCESSES HAVE BEEN CHECKED. IF NO PROCESS NEED IS LESS THAN WORK THEN THERE
		// IS NO SOLUTION.
